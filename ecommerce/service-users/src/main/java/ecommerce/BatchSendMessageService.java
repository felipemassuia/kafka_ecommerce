...
public class BatchSendMessageService {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		try (var service = new KafkaService<>("SEND_MESSAGE_TO_ALL_USERS", batchService::parse,
				CreateUserService.class.getSimpleName(), String.class, new HashMap<>())) {
			service.run();
		}
	...
	void parse(ConsumerRecord<String, Message<String>> record) throws InterruptedException, ExecutionException, SQLException {
		...
		for (User user : getAllUsers()) {
			dispatcher.send(message.getPayload(), user.getUuid(), user);
		}
